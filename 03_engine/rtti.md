...
RTTI 는 런타임에 타입의 정보를 알 수 있는 기능이다.
RTTI 의 작동 방식은 매우 간단하다.
런타임에 확정되는 **타입 정보를 어딘가에 저장**해두고, 타입 식별이 필요할 때 꺼내쓰는 형태이다.
이 글에서는 런타임에 확정되는 정보를 Run-Time Meta Data 라고 칭하겠다.
...
public:
    ...
    void* GetFuncPtr() const;   // 실제 함수 포인터
...
그럼에도 블루프린트에서 C++ 의 함수를 호출할 수 있는 이유는 UFunction 이라는 런타임 메타데이터가 있고, 이를 바탕으로 함수 주소를 알 수 있기 때문이다.