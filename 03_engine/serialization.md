...
유니티를 하다보면 Serialization(직렬화)이란 게 익숙하다. 정확히 어떤 개념인지는 몰라도 이것이 inspector 창에 띄우기 위해 필요하다는 것은 안다.

직렬화는 객체의 상태를 저장하거나 전송할 수 있는 형태로 변환하는 과정이다. 반대로 저장된 데이터를 다시 객체의 상태로 복원하는 과정은 Deserialization(역직렬화) 라고 한다.
...
## 직렬화와 Garbage Collection

Garbage Collection 은 어떤 객체가 아직 참조되고 있는지를 알아야 한다.
